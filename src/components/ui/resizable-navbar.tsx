...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[60] mx-auto hidden w-full max-w-7xl flex-row items-center justify-between self-start rounded-2xl bg-transparent px-4 py-3 lg:flex",
        visible && "bg-background/90 dark:bg-background/90 border border-border/50",
        className,
      )}
    >
      {children}
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          exit={{ opacity: 0, y: -20 }}
          className={cn(
            "absolute inset-x-0 top-16 z-50 flex w-full flex-col items-start justify-start gap-4 rounded-xl bg-card/95 backdrop-blur-xl px-6 py-8 shadow-xl border border-border/50",
            className,
          )}
        >
          {children}
...
export const MobileNavToggle = ({
  ...
  return (
    <button
      onClick={onClick}
      className="h-10 w-10 rounded-xl hover:bg-accent/50 flex items-center justify-center transition-colors"
    >
      {isOpen ? (
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-xl text-sm font-semibold relative cursor-pointer transition-all duration-200 inline-flex items-center justify-center gap-2";

  const variantStyles = {
    primary:
      "bg-gradient-to-r from-primary to-primary/80 text-primary-foreground hover:shadow-lg hover:shadow-primary/20 hover:scale-[1.02]",
