...
const cardVariants = cva("absolute will-change-transform", {
  variants: {
    variant: {
      dark: "flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-border/50 bg-card/80 p-6 backdrop-blur-md",
      light: "flex size-full flex-col items-center justify-center gap-6 rounded-2xl border border-border bg-card/80 p-6 backdrop-blur-md",
    },
  },
  defaultVariants: {
    variant: "light",
...
>(
  (
    ...
    const filter =
      variant === "light" 
        ? useMotionTemplate`drop-shadow(${dx}px ${dy}px ${blur}px hsl(var(--foreground) / ${alpha}))`
        : "none"
